...
const combine = (input, author) => { 
  return input.map((info) => { 
    var foundName = author.find(a => a.id === info.author_id)
    return [ { 
      title: info.title, 
      views: info.views, 
      author_name: foundName.first_name + ' ' + foundName.last_name
    }
    ]
  })
}

console.log(combine(input, author))
